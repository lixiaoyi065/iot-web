...
import { conformsTo, debounce } from "lodash";
import $ from "jquery"
...
class RealTime extends PureComponent{
  ...
  loadMore = () => {
    GetNextPageTags(this.state.activeNode).then(res => {
      this.setState((state) => {
        ...
        return {
          gist: JSON.parse(JSON.stringify([...state.gist, ...res.data])),
  ...
  addTags = () => {
    ...
    this.eiditTableRef.current.tableRef.current.api.addRows([tagObj])
    this.setState(state => {
      return {
